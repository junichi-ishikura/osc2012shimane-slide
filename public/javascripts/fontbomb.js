...
(function() {
  var Bomb, Explosion, Particle, config, targetTime, vendor, w, _i, _len, _ref,
  ...
  w.FONTBOMB_HIDE_CONFIRMATION = true;
  ...
  w.getOffset = function(el) {
    var body, _x, _y;
    body = document.getElementById(config.bodyId);
    _x = 0;
    _y = 0;
  ...
  Explosion = (function() {
    ...
    function Explosion() {
      ...
      if ((_ref = this.body) != null) {
        _ref.onclick = function(event) {
    	  socket.emit('dropbomb', window.findClickPos(event));
      ...
      KeyboardJS.bind.key("r", null, function() {
        socket.emit('reset');
        //return _this.reset();
      });
    ...
    Explosion.prototype.reset = function() {
      ...
      for (_i = 0, _len = _ref.length; _i < _len; _i++) {
        bomb = _ref[_i];
        bomb.bomb.parentNode.removeChild(bomb.bomb);
      }
      this.bombs = [];
      this.blast = null;
      _ref = this.chars;
